...
public class Level extends JPanel implements ActionListener,KeyListener {
	...
	public void paintComponent(Graphics g)
	{
	    ...
	    for (int i=0;i<11;i++)
	    {
	    if(Character.getNumericValue(Properties.mapa[k].charAt(i))==0)
	    {
	    ...
	    if(flame==true){
	   	...
	   	for(int i=0;i<x;i++)
	   	{
	    		for(int k=0;k<5;k++)
	    		{
	    		System.out.println("x:"+flames.get(5*i+k).getX()+"y:"+flames.get(5*i+k).getY());
    ...
    public void keyPressed(KeyEvent e) {
    	...
    	else if (e.getKeyCode()==KeyEvent.VK_SPACE)
    	{
    		...
    		if (bombs.size()==1){
        		...
        		flames.get(5*(x-1)).initPosition(gracz.getX()-50, gracz.getY());
        		flames.get(5*(x-1)+1).initPosition(gracz.getX()+50, gracz.getY());
        		flames.get(5*(x-1)+2).initPosition(gracz.getX(),gracz.getY()+50);
        		flames.get(5*(x-1)+3).initPosition(gracz.getX(), gracz.getY()-50);
        		flames.get(5*(x-1)+4).initPosition(gracz.getX(), gracz.getY());
    			fields.addAll(bombs);
